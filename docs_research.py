"""Documentation research via OpenAI web search + direct HTTP fetch.
...
# --------------------------------------------------------------------------- #
# OpenAI web search (native Responses API `web_search` tool, one paid provider)
# --------------------------------------------------------------------------- #
_SEARCH_SYSTEM = (
...
def _search_cache_path(queries: list[str], k: int):
    payload = json.dumps({"v": 2, "queries": queries, "k": k}, sort_keys=True)
    digest = hashlib.sha256(payload.encode("utf-8")).hexdigest()
    return config.CACHE_DIR / "openai_search" / f"{digest}.json"
...
def _cached_search(queries: list[str], k: int) -> list[dict] | None:
    ...
    try:
        ...
        return None
    max_age = dt.timedelta(days=float(os.getenv("SEARCH_CACHE_DAYS", "7")))
    if dt.datetime.now(dt.timezone.utc) - generated > max_age:
        return None
    return cached.get("results") or []


def _citations_from_response(response) -> list[dict]:
    """Pull url_citation annotations (real, search-sourced URLs) from a Response."""
    found: list[dict] = []
...
def _search_one(query: str, k: int) -> tuple[list[dict], float]:
    ...
    try:
        obj = config._extract_json_object(text)
    ...
    # Backfill / cross-check with the real url_citation annotations from search.
    by_url = {r["url"]: r for r in parsed_results}
    for citation in _citations_from_response(response):
        existing = by_url.get(citation["url"])
        if existing is None:
