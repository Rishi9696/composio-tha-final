...
def _preflight_paid_runtime(workers: int) -> None:
    ...
    try:
        ...
        errors.append(f"openai is not installed in {sys.executable}")
    if workers < 1 or workers > config.OPENAI_MAX_WORKERS:
        errors.append(
            f"--workers must be 1..{config.OPENAI_MAX_WORKERS}"
        )
    if errors:
...
def main() -> None:
    ...
    p.add_argument("--workers", type=int, default=config.OPENAI_MAX_WORKERS)
    p.add_argument("--model", help="override the OpenAI model")
    p.add_argument(
