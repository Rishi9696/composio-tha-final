...
The agent deliberately cannot write any project artifact. It gives the OpenAI
model three preloaded local tools: load one verified record, run one first-party
Browser Tool investigation, and deterministically compare the returned verdict.
"""
...
def _function_arguments(tool_call) -> dict:
...
def _without_schema_examples(value):
    """Strip the ``examples`` keyword recursively for maximally portable schemas."""
    if isinstance(value, dict):
        return {
...
def _sanitize_tool_schemas(tools: list) -> list:
    """Remove the ``examples`` keyword from each OpenAI tool's parameter schema."""
    sanitized = []
    for tool in tools:
        if isinstance(tool, dict) and isinstance(tool.get("function"), dict):
            function = dict(tool["function"])
            if "parameters" in function:
                function["parameters"] = _without_schema_examples(function["parameters"])
            sanitized.append({**tool, "function": function})
        else:
...
def _openai_provider():
    """Composio's OpenAI provider adapts the executor to OpenAI tool schemas."""
...
def run(
    ...
    *,
    ...
    client = openai_client or config.get_client()
    wrapped_tools = _sanitize_tool_schemas(list(session.tools()))
    system_instruction = (
    ...
    for index, expected_tool in enumerate(sequence):
        if index + 1 > max_rounds:
            raise RuntimeError(f"exceeded the {max_rounds}-round tool-call limit")
        completion = _complete(expected_tool)
        ...
        if name != expected_tool:
            raise RuntimeError(f"model called {name or '<unnamed>'}; expected {expected_tool}")
        trace.append(name)
        print(f"[composio-agent] session={session.session_id} tool={name}")
        executed = session.execute(name, arguments=_function_arguments(tool_call))
    ...
    summary = _complete(None)
    last_message = summary.choices[0].message
    return {
        "slug": slug,
        "model": selected_model,
        ...
        "agent_summary": (getattr(last_message, "content", "") or "").strip(),
        "read_only": True,
    }
